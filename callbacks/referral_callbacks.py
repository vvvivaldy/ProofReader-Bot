...
async def referral_callback(callback: types.CallbackQuery):
    ...
    if callback.data == 'get_partnership':
        ...
        link = cursor.execute(f'SELECT link FROM referral WHERE id = {callback.from_user.id}').fetchone()[0]
        profit = cursor.execute(f'SELECT profit FROM referral WHERE id = {callback.from_user.id}').fetchone()[0]
