...
"""
        ...
        case 'pushOrder':
            ...
            await callback.message.edit_text(text='У ваших подписчиков в данный момент есть открытый вами ордер на данной валютной паре. Вероятно,'\
                                                'Вы хотите докупить и/или изменить стоп-ордера. Вы хотите отправить им ТОЛЬКО ЧТО СОЗДАННЫЙ ВАМИ ордер,'\
                                                'или не будете?\n\n\n ВЫ ОТПРАВИЛИ ОРДЕР✅')
...
async def activation_quantity(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        cursor.close()
        await bot.send_message(chat_id=message.from_user.id,
                               text="Напишите дату, до которой будет работать ключ в формате дд-мм-гггг или напишите <code><b>Бессрочно</b></code>, чтобы ключ работал всегда.",
                               parse_mode="HTML")
        await state.reset_state()
        await Key_Duration.date.set()
    except ValueError:
    ...
    try:
        if date.title() != "Бессрочно":
            ...
            if a >= datetime.now().date():
                ...
                await bot.send_message(chat_id=message.from_user.id,
                                       text=f"Ваш ключ: <code><b>{key}</b></code> ! Отправьте его пользователям, чтобы они смогли отслеживать ваши действия.",
                                       parse_mode="HTML",
                                       reply_markup=kb_trader)
            else:
                cursor.execute(f"DELETE FROM trader_keys WHERE key = '{key}'")
